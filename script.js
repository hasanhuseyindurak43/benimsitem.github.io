// script.js - DÜZELTİLMİŞ (OTOMATİK REKLAM ENGELLENDİ)
document.addEventListener('DOMContentLoaded', () => {

    // --- DEĞİŞKENLER VE SEÇİCİLER ---
    const STORAGE_KEY = 'hesapData';
    const AD_COOLDOWN_MINUTES = 5; // 5 dakika bekleme
    const AUTO_AD_INTERVAL_MINUTES = 7; // 7 dakikada bir otomatik reklam
    const CREDIT_TO_USDT_RATE = 0.000000001; // 1 Kredi = 0.000000001 USDT
    
    // Her reklamda 1 veya 2 kredi kazanacak
    const MIN_REWARD_CREDITS = 1;   // 1 Kredi
    const MAX_REWARD_CREDITS = 2;   // 2 Kredi
    ...
    if (window.Telegram) {
        ...
        if (user) {
            ...
            // NOT: Monetag script'i artık önceden yüklenmiyor
            // Otomatik reklam zamanlayıcısını başlat
            startAutoAdTimer();
        } else {
    ...
    // --- DÜZELTİLMİŞ: MONETAG SCRIPT'İNİ YÜKLEME FONKSİYONU ---
    function loadMonetagScript(showAdAfterLoad = true) {
        return new Promise((resolve, reject) => {
            // Eğer script zaten yüklüyse, önce kaldır
            ...
            // Script yüklendikten sonraki işlemler
            script.onload = () => {
                monetagScriptLoaded = true;
                console.log('Monetag scripti başarıyla yüklendi.');
                
                // Script yüklendikten sonra reklam göstermek için kısa bir gecikme
                setTimeout(() => {
                    if (showAdAfterLoad && window.show_10301465) {
                        showAd();
                    }
                    resolve();
                }, 500);
    ...
    // --- YENİ: MONETAG SCRIPT'İNİ KALDIRMA FONKSİYONU ---
    function removeMonetagScript() {
        const existingScript = document.querySelector(`script[src="${MONETAG_CONFIG.scriptUrl}"]`);
        if (existingScript) {
            existingScript.remove();
            monetagScriptLoaded = false;
            console.log('Monetag scripti kaldırıldı.');
        }
    }

    // --- DÜZELTİLMİŞ: SADECE MONETAG REKLAMINI GÖSTERME FONKSİYONU ---
    function showMonetagAd() {
        ...
        if (now < userData.nextAdTimestamp) {
            ...
            tg.showAlert(`Lütfen bekleyin. Bir sonraki reklam için ${minutes}:${seconds.toString().padStart(2, '0')} kaldı.`);
            return;
        }

        // Monetag script'ini yükle ve reklamı göster
        loadMonetagScript(true)
            .then(() => {
                console.log('Reklam yüklendi ve gösteriliyor.');
            })
            .catch(error => {
    ...
    function showAd() {
        ...
        adMessage.innerHTML = 'Lütfen reklamı tam olarak izleyin<br>' +
                             'Kazanacağınız: 1 veya 2 Kredi<br>' +
                             '(1 Kredi = 0.000000001 USDT)<br>' +
        ...
        adTimer.textContent = 'Reklam izleniyor... Kalan süre: 45 saniye';
        ...
        monthlyInfo.innerHTML = '<strong>Kazanç Bilgisi:</strong><br>' +
                               '• Her reklam: 1 veya 2 Kredi<br>' +
                               '• 1 Kredi = 0.000000001 USDT<br>' +
                               '• 5 dakika bekleme süresi<br>' +
                               '• Sadece Monetag reklamları';
        ...
        adWatchInterval = setInterval(() => {
            ...
            if (secondsLeft <= 0) {
                ...
                // Reklam bittikten sonra script'i kaldır
                setTimeout(() => {
                    removeMonetagScript();
                }, 1000);
            }
        ...
        try {
            if (window.show_10301465) {
                ...
                clearInterval(adWatchInterval);
                document.body.removeChild(adOverlay);
                tg.showAlert('Reklam gösterilirken bir hata oluştu.');
            }
        } catch (error) {
    ...
    // --- YENİ: OTOMATİK REKLAM ZAMANLAYICISI ---
    function startAutoAdTimer() {
        // 7 dakikada bir otomatik reklam göster
        autoAdInterval = setInterval(() => {
            showAutoAd();
        }, AUTO_AD_INTERVAL_MINUTES * 60 * 1000);
        
        // İlk otomatik reklam için de zamanlayıcı başlat
        setTimeout(() => {
            showAutoAd();
        }, AUTO_AD_INTERVAL_MINUTES * 60 * 1000);
    }
    ...
    function showAutoAd() {
        // Eğer kullanıcı banlıysa veya başka sayfadaysa otomatik reklam gösterme
        if (userData.isBanned || !homePage.style.display || homePage.style.display === 'none') {
            return;
        }
        
        console.log('Otomatik reklam gösteriliyor...');
        
        // Otomatik reklam için Monetag script'ini yükle
        loadMonetagScript(true)
            .then(() => {
                console.log('Otomatik reklam yüklendi.');
            })
            .catch(error => {
                console.error('Otomatik reklam yüklenemedi:', error);
            });
    }

    function handleAdSuccess() {
        // Rastgele 1 veya 2 kredi kazanma
        ...
        tg.showAlert(`Tebrikler! ${earnedCredits} Kredi (${earnedUSDT.toFixed(9)} USDT) kazandınız!\n5 dakika sonra tekrar reklam izleyebilirsiniz.`);
    ...
    function updateUI() {
        ...
        // Kredi bakiyesini tam sayı olarak göster
        creditBalanceSpan.textContent = userData.credit_balance;
        usdtBalanceSpan.textContent = usdtBalance.toFixed(9);
        userBalanceSpan.textContent = `${usdtBalance.toFixed(9)} USDT`;
    ...
    function updateAdButtonState() {
        ...
        if (now >= userData.nextAdTimestamp) {
            ...
            adButtonTextSpan.innerHTML = 'Reklam İzle';
        } else {
    ...
    function renderAdminUsersList() {
        ...
        for (const userId in users) {
            ...
            userRow.innerHTML = `
                <td>${user.username}</td>
                <td>${usdtBalance.toFixed(9)} USDT</td>
                <td>${statusText}</td>
                <td>
                    <div class="btn-group" role="group">
                        ...
                        <button class="btn btn-sm btn-warning" onclick="resetUserData('${userId}')">
                            Sıfırla
                        </button>
                    </div>
                </td>
        ...
        for (const userId in users) {
            if (users[userId].isBanned) {
                bannedUsers.push({
                    ...
                    usdtBalance: (users[userId].credit_balance * CREDIT_TO_USDT_RATE).toFixed(9)
                });
            }
        }
            ...
            userCard.innerHTML = `
                <div class="d-flex justify-content-between align-items-center">
                    <div>
                        ...
                        <div class="text-muted small">Bakiye: ${user.usdtBalance} USDT</div>
                    </div>
                    <div class="btn-group" role="group">
                        <button class="btn btn-success btn-sm" onclick="toggleUserBan('${user.id}')">Banı Kaldır</button>
                        <button class="btn btn-warning btn-sm" onclick="resetUserData('${user.id}')">Sıfırla</button>
                    </div>
                </div>
            `;
    ...
    window.toggleUserBan = function(userId) {
        ...
        // Eğer kullanıcı kendini banlarsa, sayfayı yeniden yükle
        if (userId === tg.initDataUnsafe.user.id.toString() && user.isBanned) {
            setTimeout(() => {
                location.reload();
            }, 1000);
        }
    ...
    // --- YENİ FONKSİYON: Kullanıcı verilerini sıfırla ---
    window.resetUserData = function(userId) {
        if (!appData.users[userId]) return;
        
        if (confirm(`${appData.users[userId].username} kullanıcısının tüm verilerini sıfırlamak istediğinizden emin misiniz?\n\nBu işlem geri alınamaz!`)) {
            const user = appData.users[userId];
            
            // Kullanıcı verilerini sıfırla
            user.credit_balance = 0;
            user.usdt_wallet_address = '';
            user.lastAdTimestamp = 0;
            user.nextAdTimestamp = 0;
            // NOT: isBanned durumu değişmeyecek
            
            saveAppData();
            renderAdminPanel();
            
            tg.showAlert(`${user.username} kullanıcısının verileri başarıyla sıfırlandı.`);
            
            // Eğer kullanıcı kendi verilerini sıfırlarsa, UI'ı güncelle
