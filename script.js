...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- YENİ: REKLAM ŞİRKETLERİ AYARLARI ---
    const AD_PROVIDERS = {
        adexium: {
        ...
        adextra: {
            ...
            containerId: 'd34bcbcf6512122c89e0c7654c0bda0619117b6e'
        },
        monetag: {
    ...
    const adContainer = document.getElementById('ad-container'); // Yeni reklam konteyneri
    ...
    // --- YENİ: REKLAM SCRIPT'LERİNİ DİNAMİK YÜKLEME FONKSİYONU ---
    function loadAdScript(providerName) {
        return new Promise((resolve, reject) => {
            ...
            const provider = AD_PROVIDERS[providerName];
            ...
            if (providerName === 'adextra') {
                ...
                script.setAttribute('data-zone', provider.dataZone);
                script.setAttribute('data-sdk', provider.dataSdk);
            }
    ...
    // --- YENİ: REKLAMI GÖSTERME FONKSİYONU ---
    function showRandomAd() {
        const providers = Object.keys(AD_PROVIDERS);
        const randomProvider = providers[Math.floor(Math.random() * providers.length)];
        
        loadAdScript(randomProvider)
            .then(() => {
                switch (randomProvider) {
                    case 'adexium':
                        ...
                        break;
                    case 'adextra':
                        // Adextra için container'ı kontrol et
                        if (!document.getElementById(AD_PROVIDERS.adextra.containerId)) {
                            const adextraContainer = document.createElement('div');
                            adextraContainer.id = AD_PROVIDERS.adextra.containerId;
                            adContainer.appendChild(adextraContainer);
                        }
                        break;
                    case 'monetag':
